...
def parse_prompt_and_answer(prompt, answer):
    ...
    # print('Original label', command, speed_content, his_trajs, future_trajs, future_trajs_pixel)
...
def transform_traj2pixel(trajs, cur_anno):
    # Convert ego vehicle coordinate system to camera coordinate system
    ...
    # location_0[2] = location_0[2] - extent[2] # move to ground level
    trajs[:, 3] = 1
    trajs[:, 2] = trajs[:, 0]
    trajs[:, 0] = 0
    # trajs[:, -2] = location_0[2]
    # Convert camera coordinate system to pixel coordinate system
...
def visual_for_bev(label, pred=None, visual_path=None):
    ...
    # Draw text and visualize points:
    ...
    print('Pixel coordinates', future_trajs_pixel, targetpointpixel)
    if pred is not None:
        pred_trajs, pred_trajs_pixel = parse_answer(pred['pred'])
    else:
        pred_trajs, pred_trajs_pixel = None, None
    # print('Extracted label', his_trajs, future_trajs, future_trajs_pixel)
    ...
    # Draw text and visualize BEV image
...
def main_for_vis_train():
    ...
    # Iterate over each subfolder under base_folder
    print(f'Start reading {train_json}')
    with open(train_json, 'r') as f:
        lines = f.readlines()
    print(f'Finished reading {train_json}')
...
def main_for_vis_infer():
    ...
    os.makedirs(visual_path, exist_ok=True)
    # Iterate over each subfolder under base_folder
    print(f'Start reading {gt_json}')
    with open(gt_json, 'r') as f:
        lines = f.readlines()
    print(f'Finished reading {gt_json}')

    print(f'Start reading {pred_json}')
    with open(pred_json, 'r') as f:
        lines_pred = f.readlines()
    print(f'Finished reading {pred_json}')
...
def print_l2_loss(losses_1s, losses_2s):
    ...
    # Print results
    print(f"\n🎯 Trajectory prediction error evaluation (1s and 2s only)")
    print(f"{'Period':<6} {'Samples':<8} {'Mean L2 Error':<12} {'Std Dev':<10} {'Min':<8} {'Max':<8}")
    print("-" * 60)
